...
public class Testing{
    ...
    public static void main(String[] args){
        ...
        if(test4){
            System.out.printf("\n%-100s\t%s[OK]%s\n", testsRef.get(tests-1).getKey(), ANSI_GREEN, ANSI_RESET);
            count++;
        }
        else{
            System.out.printf("\n%-100s\t%s[Fail]%s\n", testsRef.get(tests-1).getKey(),ANSI_RED, ANSI_RESET);
        }
        if(count == tests){
        ...
        try{
            ...
            while (read.hasNextLine()){
                ...
                if(line.startsWith("Test case")){
                    test = new ArrayList<>();
                    tests.add(new Entry<String,ArrayList<String>>(line, test));
                }
                else{
                    if(test != null)
                        test.add(line);
